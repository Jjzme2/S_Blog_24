component name="MarkdownGenerationService" {
	...
	public function addNewTitle(
		...
		appendToFile( fileName = fileName, content = content );
	}
	...
	public function getTitleById( required string hyphenSepareatedID ){
		...
		if ( len( trim( arguments.hyphenSepareatedID ) ) ) {
			var id             = replace( arguments.hyphenSepareatedID, "-", " ", "all" );
			var idRegex        = "id='" & id & "'";
			var idRegexMatches = reFind( arguments.idRegex, arguments.fileContent );

			if ( arrayLen( arguments.idRegexMatches ) ) {
				var titleRegex        = chr( 35 ) & " " & id & ".*";
				var titleRegexMatches = reFind( arguments.titleRegex, arguments.fileContent );
	...
	public function addLink(
		...
		var content = "[" & arguments.contentToAdd & "](" & arguments.link & ")" & chr( 10 );
		appendToFile( fileName = fileName, content = content );
	}
	...
	public function addCode(
		...
		var content           = "";
		if ( arrayFind( arguments.acceptedCodeTypes, arguments.codeType ) ) {
	...
	public function addList(
		...
		var references = {
			"ordered"   : [ "ordered", "numbered", "o", "ord" ],
			"unordered" : [ "unordered", "bullet", "u", "unord" ]
		};
