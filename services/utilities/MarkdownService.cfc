component name="markdownService" {
	...
	struct function extractFrom( required string markdownContent ){
		...
		var metaData = trim(
			replace(
				reFind( metaRegex, markdownContent, 1, 1, "all" )[ 1 ].match[ 1 ],
				"---",
				"",
				"all"
			)
		...
		var styleData = trim(
			replace(
				reFind( styleRegex, markdownContent, 1, 1, "all" )[ 1 ].match[ 1 ],
				"<style>",
				"",
				"all"
			)
		...
		var commentData = trim(
			replace(
				reFind( commentRegex, markdownContent, 1, 1, "all" )[ 1 ].match[ 1 ],
				"<!--",
				"",
				"all"
			)
		...
		var content = reReplace( markdownContent, metaRegex, "" );
		content     = reReplace( content, styleRegex, "" );
		content     = reReplace( content, commentRegex, "" );
	...
	string function clean( required string v ){
		...
		if ( cleanedValue == "" ) {
			throw( type = "Application", message = "The value contained an empty string or only whitespace" );
		}
		return cleanedValue;
	}
