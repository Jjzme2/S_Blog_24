component name="StringService" {
	 ...
	 */
	public string function convertToCaseAndTrim( required string contentToConvert, string case = "lower" ){
		switch ( arguments.case ) {
			...
			case "sentence":
				return uCase( left( trim( arguments.contentToConvert ), 1 ) ) & lCase(
					mid( trim( arguments.contentToConvert ), 2 )
				);
			default:
