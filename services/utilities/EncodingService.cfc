	...
	singleton
{
	...
	public function getASCIIEncoding( string keyToEncode ){
		...
		if ( keyToEncode EQ nullValue() || len( keyToEncode ) EQ 0 ) {
			throw(
				message = "The key to encode cannot be null or empty.",
				detail  = "EncodingService.getASCIIEncoding",
		...
		if ( !ASCIICodes.keyExists( keyToEncode ) ) {
			throw(
				message = "The key to encode (#keyToEncode#) is not a valid key.",
				detail  = "Valid Keys: #arrayToList( getASCIIKeys() )#",
				type    = "EncodingService"
			);
		}
