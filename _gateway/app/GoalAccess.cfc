					...
					(
						SELECT
							JSON_ARRAYAGG(
								JSON_OBJECT(
									'taskId', st.id,
									'taskCompleted', CAST(COALESCE(st.is_complete, 0) AS SIGNED),
									'taskName', st.name,
									'taskDescription', st.description,
									'taskFollowUpNotes', st.follow_up_notes
								)
							)
					) AS tasks,
					(
						SELECT
							JSON_ARRAYAGG(
								JSON_OBJECT(
									'noteId', n.id,
									'noteName', n.name,
									'noteDetail', n.description
								)
							)
						FROM notes n
						WHERE t.notes IS NOT NULL AND FIND_IN_SET(n.id, t.notes) > 0
					) AS notes
				FROM #tableName# t
				LEFT JOIN statuses s ON t.status_id = s.id
				LEFT JOIN motivations m ON t.motivation_id = m.id
				LEFT JOIN tasks st ON st.goal_id = t.id
