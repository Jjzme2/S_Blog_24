/**
 * @Author      Jj Zettler
 * @Description This will be the access point for the note table.
 * @date        12/9/2023
 * @version     0.1
 * @FindOBJECT  note
 * @FindCOLUMNS t.id
...
<cfcomponent output="false" extends="BaseAccess">
	...
	>
		...
		<cfcatch type="any">
			<cfset var messages = ["note Access GETBYACTIVITYSTATUS", cfcatch.message]>
	...
	<cffunction name="create" access="package" returntype="any" output="false" hint="Adds a new entry into the database.">
		<cfargument name="entity" type="noteDTO" required="true">
		...
		<cftry>
			<cfquery name="qry" datasource="#dataSource#">
				...
				<cfset var qry = get(
					...
					<cfreturn new QueryHandler( qry )>
				<cfcatch>
					<cfset var messages = ["note Access CREATE", cfcatch.message]>
					<cfthrow type="CustomError" message=#serializeJSON(messages)#>
					<cfreturn false>
				</cfcatch>
		</cftry>
	 ...
	 */
	 ...
	 <cffunction name="get" access="private" returntype="any" output="false" hint="Gets an entity by the key-value pairs provided">
		...
		<cftry>
			...
			<cfcatch type="any">
				<cfset var message = {
					"customMessage": "Error occurred in note Access GET.",
