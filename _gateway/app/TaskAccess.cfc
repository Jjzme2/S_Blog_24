...
<cfcomponent output="false" extends="BaseAccess">
	...
	<cfset tableName  = "tasks">
		<cfset dataSource = application.cbController.getSetting( 'secondaryDatasource2' )>
		...
		<cftry>
			<cfquery name="qry" datasource="#dataSource#">
				...
				VALUES(
					...
					,<cfqueryparam value="#entity.getIsComplete()#" cfsqltype="cf_sql_bit">
					...
					,<cfqueryparam value="#entity.getFollowUpNotes()#" cfsqltype="cf_sql_varchar">
					,<cfqueryparam value="#entity.getGoalId()#" cfsqltype="cf_sql_varchar">
				);
				</cfquery>
				...
				<cfset var qry = get(
					...
					<cfreturn new QueryHandler( qry )>
				<cfcatch>
					<cfset var messages = ["task Access CREATE", cfcatch.message]>
					<cfthrow type="CustomError" message=#serializeJSON(messages)#>
					<cfreturn false>
				</cfcatch>
		</cftry>
	 ...
	 */
	 ...
	 <cffunction name="get" access="private" returntype="any" output="false" hint="Gets an entity by the key-value pairs provided">
		...
		<cftry>
			<!---
				 ! Follow Up Notes seems to be a CSV list of notes, not from the notes in the database.
			--->
			<cfquery name="qry" datasource="#dataSource#">
				SELECT t.id
				...
				,t.name
				,t.description
				,t.follow_up_notes as followUpNotes
				,t.goal_id as goalId
